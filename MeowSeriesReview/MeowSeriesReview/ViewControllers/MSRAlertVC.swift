...
class MSRAlertViewVC: UIViewController {
	
	let containerView   = UIView()
	let titleLabel      = GFTitleLabel(textAlignment: .center, fontSize: 20)
	let messageLabel    = GFBodyLabel(textAlignment: .center)
	let actionButton    = GFButton(backgroundColor: .systemGreen, title: "OK")
	let cancelButton    = GFButton(backgroundColor: .secondaryLabel, title: NSLocalizedString("Cancel", comment: ""))
...
class AlertWithBigInfoVC: UIViewController {
	
	let containerView   		  	= UIView()
	let tomorrowAmountContainer  	= UIView()
	let titleLabel      			= GFTitleLabel(textAlignment: .center, fontSize: 20)
	let tomorrowAmountDescription  	= GFTitleLabel(textAlignment: .left, fontSize: 11)
	let tomorrowAmountLabel  		= GFTitleLabel(textAlignment: .center, fontSize: 24)
	let messageLabel    			= GFBodyLabel(textAlignment: .left, fontSize: 11)
	let actionButton    			= GFButton(backgroundColor: .systemGreen, title: "OK")
	let cancelButton    			= GFButton(backgroundColor: .secondaryLabel, title: NSLocalizedString("Cancel", comment: ""))
	...
	let padding: CGFloat = 20
	var completion: (() -> Void)?
	var tomorrowAmountText: String?
	
	
	init(title: String, message: String, okButtonTitle: String, tomorrowAmountText: String, _ completion: @escaping() -> Void) {
		super.init(nibName: nil, bundle: nil)
		self.alertTitle     		= title
		self.message        		= message
		self.okButtonTitle			= okButtonTitle
		self.tomorrowAmountText		= tomorrowAmountText
		self.completion 			= completion
	}
	...
	func configureMessageLabel() {
		...
		NSLayoutConstraint.activate([
			messageLabel.bottomAnchor.constraint(equalTo: tomorrowAmountContainer.topAnchor, constant: -45)
		])
